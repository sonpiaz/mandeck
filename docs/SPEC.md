...
| R-30 | Accent mutability (v0.1.2 owner ruling, supersedes the "immutable thereafter" clause of R-28). | The settings swatch row recolors the ACTIVE workspace: clicking a swatch sets that workspace's `accentHue` at runtime, applied live (focus border, active chip, drop wash) and persisted by the normal debounced save. Creation-time auto-rotation (R-28 scan) is unchanged for new workspaces; `settings.defaultAccent` remains the rotation seed, file-only. |
| R-31 | Pane view types (v0.1.2). | Panes carry a view type, terminal or files, stored as the OPTIONAL root map `paneViews: {pid: "files"}` in state.json v2 — only non-terminal entries stored, absence/invalid → terminal, hydration never rejects, no version bump. The files pane is a full grid citizen (drag, move-to-workspace, maximize, close); its current directory lives in `paneCwds[pid]` like a terminal cwd. |
